...
library(pacman)
p_load(leaflet, ggplot2, ggthemes, viridis, dplyr, magrittr, knitr,pagedown,cowplot,latex2exp,parallel,xtable)
# Define pink color
red_pink <- "#e64173"
...
- Reading quiz next week - submit a question
- HW1 back today, regrade due next Tuesday 10/29
...
**Part 1: Short answer questions (~3)**
  ...
  + Be able to interpret probability distributions, scatter plots, boxplots, linear regression output (not $p$-values or $t$-statistics)
...
Interpret the following regression output. 

1. Write the regression equation.
2. What are the values of the coefficients?
3. What do the coefficients mean in plain language?
...
```{R, transfigure6, dev = "svg", fig.height = 6.5, echo = F}
# Plot
ggplot(data = trans_df, aes(x = x, y = y)) +
geom_line(stat = "smooth", method = lm, formula = y ~ 1, color = "orange", size = 1.5, alpha = 0.3) +
geom_line(stat = "smooth", method = lm, formula = y ~ x, color = "orange", size = 1.5, alpha = 0.3) +
