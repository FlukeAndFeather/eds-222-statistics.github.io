...
```{r}
lm_size <- lm(size ~ latitude, pie_crab)
sse <- sum(residuals(lm_size)^2)
size_mean <- mean(pie_crab$size)
sst <- sum((pie_crab$size - size_mean)^2)
r2 <- 1 - sse / sst
r2
...
ggplot(mpg, aes(x = year_fct, y = hwy)) +
  geom_boxplot()
