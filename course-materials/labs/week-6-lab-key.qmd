...
1.  How would you fit a model that includes and interaction term between ethnicity and SES?
2.  Recall for continuous x continuous OLS interactions, we put one continuous predictor on the x-axis and added lines for the predicted response *at different values of the second continuous predictor*. Do the same here. Plot the predicted probability of a drinking water violation (response) with respect to percent Hispanic population (first continuous predictor) at low, medium, and high levels of median household income (second continuous predictor).
3.  Interpret the predicted lines. Does SES influence the relationship between ethnicity and exposure to environmental hazards?
