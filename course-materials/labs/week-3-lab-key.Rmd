...
$$\text{crab size} = \beta_0 + \beta_1 \text{latitude} + u$$
and:


$$\text{water temperature} = \beta_0 + \beta_1 \text{latitude} + u$$
...
```{r}
summary(lm(size ~ latitude, pie_crab))$r.squared
summary(lm(water_temp ~ latitude, pie_crab))$r.squared
```
...
$$hwy_i=\beta_{0}+\beta_{1} \cdot vintage_i + u$$
...
```{r}
summary(lm(hwy ~ displ + year_fct, mpg))
```
...
```{r, echo=TRUE, cache=T}
mpg_mod <- lm(hwy ~ displ + year_fct, mpg)
```
...
ggplot(mpg, aes(displ, hwy, color = year_fct)) +
  geom_point() +
  geom_line(aes(y = .fitted), augment(mpg_mod))
```
